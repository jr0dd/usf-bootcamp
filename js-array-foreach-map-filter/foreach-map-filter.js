...
function doubleValues (arr) {
  const newArr = []
  arr.forEach(int => newArr.push(int * 2))
  return newArr
}
...
function find (arr, searchValue) {
  const newArr = arr.filter(item => item === searchValue)
  return newArr[0]
}
...
function findInObj (arr, key, searchValue) {
  const newObj = arr.filter(obj => obj[key] === searchValue)
  return newObj[0]
}
...
function removeVowels (str) {
  const vowels = ['a', 'e', 'i', 'o', 'u']
  const word = str.toLowerCase().split('')
  const stripped = word.filter(item => !vowels.includes(item))
...
function doubleOddNumbers (arr) {
  return arr.filter(int => int % 2 !== 0).map(int => int * 2)
}
